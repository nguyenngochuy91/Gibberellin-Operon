...
def parser_code():
    ...
    parser.add_argument("-o", "--outfolder", dest="outfolder", metavar="DIRECTORY", default='./new_tree/',
            ...
            for fnum, feature in enumerate(seq_record.features):
                ...
                #start = feature.location._start.position
                    ...
                    if region.lower() == marker.lower():
                        ...
                        #result.append(">%s|%s|%s" % (accession, organism, gene))
                        #print organism+seq
                        result.append(">%s" % (accession)) #make sure they are different
                        result.append(''.join(seq))
                        orgs_with_marker.append(accession)
                        found = True
                        break
...
def return_tree_order_list_2(newick_tree_file, common_to_accession_dict, accession_to_common_outfile, phylo_order_new_outfile):
    result = []
    # print "common_to_accession_dict",common_to_accession_dict
...
def main():
    ...
    if tree_file == 'NONE':
        ...
        #print "got here 1"
        common_to_accession_dict = make_target_fasta(marker_gene, genbank_directory, filter_file, marker_fasta,has_blocks)
        #print "got here 2"
