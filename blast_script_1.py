...
def parser_code():
    ...
    parser.add_argument("-o", "--outfolder", dest="outfolder", metavar="DIRECTORY", default='./blast_result_1/',
...
def do_parallel_blast(arg_tuple):
    ...
    out_file = "%s%s.txt" % (blast_result_folder, db.split('/')[-1].split('.ffc')[0])
    ...
    if 'reference' not in out_file and "Rhizobium_etli_CFN_42" not in out_file :
        cmd = "blastp -query %s -db %s -evalue %s -out %s -num_threads 8 -max_target_seqs 2 -outfmt 6" % (query_file, db, eval_threshold, out_file)
    elif "Rhizobium_etli_CFN_42" in out_file:
        cmd = "blastp -query %s -db %s -evalue %s -out %s -num_threads 8 -max_target_seqs 1 -outfmt 6" % (query_file, db, '1e-10', out_file)
    elif 'reference'  in out_file:
        cmd = "blastp -query %s -db %s -evalue %s -out %s -num_threads 8 -max_target_seqs 1 -outfmt 6" % (query_file, db, '1e-38', out_file)
    os.system(cmd)
    #print (cmd)
...
#    print ("blast_arg_list",blast_arg_list)
    pool = Pool(processes = num_proc)
    pool.map(do_parallel_blast, blast_arg_list)
    
def blast_parse(blast_dir):
    result = returnRecursiveDirFiles(blast_dir)
    outfile = open('blast_parse_1/gibberellin.txt','w')
    for item in result:
        infile = open(item,'r')
