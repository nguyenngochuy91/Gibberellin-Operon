...
def parser_code():
    ...
    parser.add_argument("-i", "--infolder", dest="infolder", default='./blast_result_1/', metavar="DIRECTORY",
                help="A file that contains the path to every organism database that you are interested in.")
    
    parser.add_argument("-o", "--outfolder", dest="outfolder", metavar="DIRECTORY", default='./blast_parse_1/',
                help="Folder where the BLAST results will be stored. Default is the folder './blast_parse_1/'.")
...
def parallel_blast_parse_dict(in_folder, out_folder, num_proc, filter_file, gene_block_dict):
    ...
    for fname in fname_list:
        for line in [i.strip() for i in open(fname).readlines()]:
            ...
            predicted_gene = hlog.query_locus()
    ...
    for gene_block in result.keys():
        ...
        for accession in result[gene_block].keys():
            if accession =="CP003057.2":
                print result[gene_block][accession]
            handle.write('\n'.join(result[gene_block][accession]) + '\n')
        handle.close()
