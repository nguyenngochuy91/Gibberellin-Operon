...
'''
def reduce_gene(gene_block,data,genes):
    result=[]
    # print data
...
'''@function: help with minimize split
   @input   : children_blocks,node.initial,to_add
   @output  : return what to do for node initial
''' 
def split_help(children_blocks,initial,to_add):
    size = len(initial)
    for gene in to_add:
        d = {}
        check = False
        # iterate through the block in initial to see which block to add this gene 
        # guarantee that those block dont have the gene
        for i in range(size):
            block = initial[i]
            # check block in 
            c_max = 0
            for b in children_blocks[0]:
                m = len(set(b).intersection(set(block)))
                c_max = max(c_max,m)
            for b in children_blocks[1]:
                m = len(set(b).intersection(set(block)))
                c_max = max(c_max,m)    
            if c_max:
                check = True
...
'''
def minimize_split(rooted_tree,genes):
        ...
        if not node.is_leaf():
            ...
            for child in children:
                ...
                if child.is_leaf(): # check if the child is a leaf
                    ...
                    print (child.initial)
                    children_blocks.append(reduce_gene(child.initial,gene_set,genes))
            ...
            appear = set([key for key in gene_set if gene_set[key] ==1 and key !="split"])
            initial_genes =set()
            for block in node.initial:
                for gene in block:
                    initial_genes.add(gene)
            to_add = sorted(list(appear-initial_genes))
            if len(node.initial) == 0:
                node.initial = ''.join(to_add)
            else:
#                print ("children_blocks",children_blocks)
#                print ("node.initial",node.initial)
#                print ("to_add",to_add)
                node.initial = split_help(children_blocks,node.initial,to_add)
#                print ("node.initial",node.initial) 
...
if __name__ == "__main__":
    ...
    rooted_tree = initialize_block_number(rooted_tree,leaves,genes)
